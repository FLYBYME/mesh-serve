...
| ~~`partVersion.kernel`~~ | *"the only thing standing between a stale part and a browser"* | **Resolved in F5**: Read and enforced by `checkComposition` in `cdn.compose`. A release serving a kernel outside a part's declared range produces a fatal `kernel_mismatch` composition problem that refuses composition. An absent range is accepted on parts (preserving backward compatibility for versions published before the field existed) and on the kernel artifact itself (which has no kernel). |
