...
describe('a ticket is identity, not authority', () => {
    it('reads roles from the user, so a change since issue applies', async () => {
        ...
        await node.store.upsertRole({ key: 'operator', name: 'Operator', scope: 'cluster', builtin: false, inherits: [] });
    ...
    it('answers whether a set of roles permits a contract', async () => {
        const node = await boot();
        await node.store.upsertRole({ key: 'author', name: 'Author', scope: 'cluster', builtin: false, inherits: [] });
...
describe('whoami', () => {
    it('reports the caller the API resolved, and their organizations', async () => {
        ...
        await node.store.upsertRole({ key: 'owner', name: 'Owner', scope: 'organization', builtin: false, inherits: [] });
