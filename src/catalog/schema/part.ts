...
export const partSchema = z.object({
  ...
  artifactId: z.string().optional().describe('The serve.artifact this service runs. serve.part.start loads exactly this build and nothing else, so a new build never goes live on its own -- deploying or rolling back is setting this (serve.part.update), after which serve.part.reconcile restarts the service on it. Must be a successful build of this same part. Absent only on parts from before builds were pinned: start then falls back to the newest successful build. Only meaningful for kind: "service"'),
