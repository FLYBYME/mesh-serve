 ...
 * from a clean slate. `start` boots a node; `bootstrap` claims one; `generate` renders a typed
 * client straight from the mesh; everything else (login and beyond) is rebuilt from here as it's
 * needed, going through the api like an ordinary caller.
 */
async function main(): Promise<void> {
    const program = new Command();
    program
        .name('mesh-serve')
        .description('mesh-serve -- start a node, claim a fresh one, or generate a typed client.')
        .exitOverride();
