...
export const holdSchema = z.object({
    ...
    requestedAt: z.string().describe('When the call was placed on hold, as an ISO timestamp'),
    status: z.enum(['held', 'released', 'rejected', 'expired']).default('held'),
    expiresAt: z.string().describe('When a still-held row is treated as expired, as an ISO timestamp'),
    decidedBy: z.string().optional().describe('The account that released or rejected this'),
    decidedAt: z.string().optional().describe('When it was decided, as an ISO timestamp'),
