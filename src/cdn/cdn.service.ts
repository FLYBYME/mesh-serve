 ...
 *
 * The pages are cached on `(siteId, releaseHash, site.updatedAt)` — the key that makes invalidation
 * correct by construction, because any of the three changing makes the cached page stale by
 * definition.
 *
 * `updatedAt` was missing until 2026-09-06 and the sentence above claimed correctness without it.
 * A page is built from the *site record* as well as the release — `api`, `title`, `description`,
 * `canonical`, `theme` — and none of those move `releaseHash`, so editing one left this node serving
...
export class CdnService extends ServiceModule {
    ...
    constructor(private readonly options: CdnServiceOptions = {}) {
        ...
        /**
         * **A deploy is not the only way a site changes.**
         *
         * Only `cdn.site_deployed` was listened for, so a node dropped its cached record when a
         * *release* changed and never when the record itself did. Everything else a site carries —
         * `api`, `title`, `description`, `canonical`, `theme` — could be edited and this node would
         * serve the old value until its TTL expired or the process restarted.
         *
         * `site.updated` is generated by `defineCrud` and was in `src/generated/events.ts` from the
         * beginning, listened for by nothing. It is the same shape as every other finding today: a
         * mechanism that exists, is correct, and is not reached.
         *
         * Found pointing a console at a new api URL. The record was written and the page kept the
         * old one — and because the page cache is keyed partly on the site's `updatedAt`, a stale
         * *record* also meant a stale *page*, so fixing that key alone changed nothing.
         */
    ...
    private async pageFile(
        ...
        /**
         * **`updatedAt`, because a page is built from the site record and not only from the release.**
         *
         * This was `${site.id}:${release.hash}`, described in the header above as *"the key that
         * makes invalidation correct by construction"*. It is not, and the gap is the whole of what
         * a page carries beyond its parts: `api`, `title`, `description`, `canonical`, `theme`,
         * `image`. None of those change `releaseHash`, so editing any of them left this node serving
         * the previous page **forever** — no TTL, no event, nothing.
         *
         * Found the hard way. A console was pointed at a new api URL, the record was written, and
         * the served page kept the old one: every call went to a hostname that resolved to a
         * *different* site, which refused the origin and answered 404 for routes it does not expose.
         * Three plausible-looking bugs, one stale string.
         *
         * The api already does this correctly — its route-table cache is keyed on the site's
         * `updatedAt` for exactly this reason. The two halves of `Host → site` disagreed about how
         * to invalidate the same record.
         */
        const key = `${site.id}:${release.hash}:${site.updatedAt?.toISOString() ?? ''}`;
