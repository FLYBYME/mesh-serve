...
// The site's grants used to live in this file, back when the CLI was the only thing that could seed
// a cluster. `site.seed` is a contract now and a browser can seed too, so the rules live where both
// reach them — two implementations of *what gate does this contract go behind* would drift, and the
// drift would be a site granting a write at `user`.
import { grantsFor } from './cdn/methods/grants.js';
...
// The site's grants moved to cdn/methods/grants.ts when site.seed became a contract: a browser can
// seed now, and two implementations of what gate a contract goes behind would drift.
